...
for Per in ['0','5','10','15','20','25','30']:
    ...
    for k in range(begintime,totaltime):
        ...
        for i in range(totalline):
            rou_z[i]=rou_z[i]+l_cro[i]/(totaltime-begintime)

    rou_all.append(rou_z)
...
plt.yticks([0,0.2,0.4,0.6],fontproperties='Arial',fontsize=36)
...
plt.savefig('rou_z_cro.png',bbox_inches='tight',dpi=1000)
plt.close()
...
for Per in ['5','10','15','20','25','30']:
    ...
    n_l=round(float(Per)/5)

    BOX=15*15*50
    N_PEG=round(BOX*float(Per)*0.01/(4*math.pi*0.8**3/3))

    if N_PEG!=0:
        for t in range(totalline):
            print(rou_all[n_l])
            rou_all[n_l][t]=rou_all[n_l][t]/N_PEG

    plt.plot(np.arange(0,50,0.1),rou_all[n_l],linewidth=4.0,color=cmap((float(Per)+5)/40))

plt.xlabel("z (nm)",size=40,labelpad=10,fontproperties="Arial")
plt.ylabel(chr(961)+' (g/(N'+u'$\mathregular{\cdot}$'+'cm'+'$\mathregular{^3}$)',labelpad=10,size=40,fontproperties="Arial")
plt.xticks(fontproperties='Arial',fontsize=36)
...
# legend=plt.legend(prop = {'size':20,'family':'Times New Roman'},frameon=False,edgecolor='black')
fig.set_size_inches(10,6)
plt.savefig('rou_z_norm_cro.png',bbox_inches='tight',dpi=1000)
plt.close()