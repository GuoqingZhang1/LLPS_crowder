...
for Per in [0,5,10,15,20,25,30]:
    ...
    f_MSD=open('../Crowd_%d/D_fitting.txt' %(Per),'r')
...
if typeCro=='1':
    ...
    plt.xticks([0.02,0.03,0.04],fontproperties="Arial",size=36)
elif typeCro=='2':
    plt.xlim(0,0.24)
    plt.ylim(0.35,1.0)
    plt.yticks([0.4,0.6,0.8,1.0],fontproperties="Arial",size=36)
    plt.xticks([0,0.06,0.12,0.18,0.24],fontproperties="Arial",size=36)


plt.ylabel(chr(961)+'$\mathregular{_{h}}$'+" (g/cm"+'$\mathregular{^3}$)',size=40,labelpad=10,fontproperties="Arial")
plt.xlabel('D'+' (nm'+'$\mathregular{^{2}}$'+'/ns)',size=40,labelpad=10,fontproperties="Arial")  
...
plt.savefig('D_vs_rou_high.png',dpi=1000,bbox_inches='tight')