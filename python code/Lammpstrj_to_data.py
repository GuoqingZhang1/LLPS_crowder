...
f=open('One_llps_30.lammpstrj','r')
line=f.readlines()
Lmax=len(line)
...
Boxl=-0.76056*100         #left right distance
Boxh=0.76056*100
Boxd=Boxh-Boxl
print(Boxd)
...
for i in range(N_m):
    for j in range(1,N_res):
        if ratom[i*N_res+j+1][1]-ratom[i*N_res+j+0][1]>140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][1]+=-Boxd
        if ratom[i*N_res+j+1][1]-ratom[i*N_res+j+0][1]<-140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][1]+=Boxd
        if ratom[i*N_res+j+1][2]-ratom[i*N_res+j+0][2]>140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][2]+=-Boxd
        if ratom[i*N_res+j+1][2]-ratom[i*N_res+j+0][2]<-140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][2]+=Boxd
        if ratom[i*N_res+j+1][3]-ratom[i*N_res+j+0][3]>140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][3]+=-Boxd
        if ratom[i*N_res+j+1][3]-ratom[i*N_res+j+0][3]<-140:
            nl=j
            for k in range(nl,N_res):
                ratom[i*N_res+k+1][3]+=Boxd
...
f_data=open('One_LLPS_PEG_%d.data' %(Per),'w')
...
f_data.write('\n')
f_data.write('-76.056 76.056 xlo xhi\n')
f_data.write('-76.056 76.056 ylo yhi\n')
f_data.write('-76.056 76.056 zlo zhi\n')
f_data.write('\n')
f_data.write('Atoms\n')
f_data.write('\n')
Rx=[]
