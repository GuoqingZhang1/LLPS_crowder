...
Matric_0=np.load(file="../../Single/Distance_%d.npy" %(Per))
# Matric=Matric/N_tamp

N_inter_0=np.load(file="../../Single/N_inter_%d.npy" %(Per))
...
for i in range(1):
    for j in range(145):
        for k in range(j+3,148):
            Intra_0[j][k]=Intra_0[j][k]+Matric_0[i*148+j][i*148+k]/(N_tamp)
            Intra_0[k][j]=Intra_0[k][j]+Matric_0[i*148+k][i*148+j]/(N_tamp)
...
plt.xticks([0,49,99,147],['1','50','100','148'],fontproperties='Arial',fontsize=36)
plt.yticks([-0.2,0.0,0.2],fontproperties='Arial',fontsize=36)
# plt.ylim(0,0.0002)
plt.xlim(-2,150)
plt.tick_params(length=7,pad=10,width = 3,direction='in')
bwith = 2 #边框宽度设置为2
...
plt.xticks([0,49,99,147],labels=['1','50','100','148'],fontproperties='Arial',fontsize=30)
plt.yticks([0,49,99,147],labels=['1','50','100','148'],fontproperties='Arial',fontsize=30)
