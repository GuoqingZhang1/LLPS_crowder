...
f_llps=open('MSD_4R_llps.txt',"r" )
line_llps = f_llps.readlines() # 以行的形式进行读取文件
Lmax_llps = len(line_llps)

MSD_4=np.zeros([3,Lmax_llps])

for i in range(Lmax_llps):
    ss=line_llps[i].split()
    MSD_4[0][i]=0.01*float(ss[0])
    MSD_4[1][i]=0.01*float(ss[1])
    MSD_4[2][i]=0.01*float(ss[2])
f_llps.close()
...
ax1 = fig.gca()
ax1.plot(0.1*np.arange(Lmax_llps),MSD_4[0],linewidth=6.0,label='IDR1')
ax1.plot(0.1*np.arange(Lmax_llps),MSD_4[1],linewidth=6.0,label='Helix')
ax1.plot(0.1*np.arange(Lmax_llps),MSD_4[2],linewidth=6.0,label='IDR2')
ax1.plot(0.1*np.arange(Lmax_llps),MSD_llps,linewidth=6.0,label='Condensate')
ax1.plot(0.1*np.arange(Lmax_llps),MSD_single,linewidth=6.0,label='Free state')

# plt.plot(xx,np.exp(y(np.log(xx),a_llps,b_llps)),linewidth=6.0,label='Condensate')
# ax1.plot(np.arange(Lmax_single),MSD_single,linewidth=6.0,color='orange',label='Free state')
# plt.plot(xx,np.exp(y(np.log(xx),a_single,b_single)),linewidth=6.0,label='Free state')
plt.xscale('log')
plt.yscale('log')
plt.xlim(0.1,0.1*Lmax_llps+10)
# plt.xlim(1,Lmax_single)
...
# plt.text(2, 2000000, 'Condensate', fontsize=28, color='darkgreen', fontproperties='Arial')
# plt.text(2, 10, 'Free state', fontsize=28, color='orange', fontproperties='Arial')
...
plt.savefig('MSD_4R_t_%d.png' %(Per),dpi=1000,bbox_inches='tight')