...
for Per in [0,5,10,15,20,25,30]:
    f_llps=open('../Crowd_%d/Pressure.txt' %(Per),"r" )
    line_llps = f_llps.readlines() # 以行的形式进行读取文件
    Lmax_llps = len(line_llps)

    tau_llps=[]
    for i in range(2,Lmax_llps):
        ss=line_llps[i].split()
        tau_llps.append((float(ss[3])-0.5*(float(ss[2])+float(ss[1])))*1.01325*0.1*0.5*50)

    tau_e.append(0.5*np.mean(tau_llps))
    std.append(0.5*np.std(tau_llps))
    Per_x.append(Per)
...
plt.ylabel(chr(947)+' (mN/m)',size=40,labelpad=10,fontproperties='calibri')
...
plt.savefig('Gamma_Per.png',dpi=1000,bbox_inches='tight')