...
mass_3R[0]=np.sum(Mass_res[0:51])
mass_3R[1]=np.sum(Mass_res[51:74])
mass_3R[2]=np.sum(Mass_res[74:148])
...
for i in range(Timeall-begintime):
    for j in range(100):
        ...
        Rg_n=0
        for k in range(51):    
            Rg_n=Rg_n+((R_pro[i][j][k][1]-np.sum(Rm[i,j,0:51,0])/mass_3R[0])**2+(R_pro[i][j][k][2]-np.sum(Rm[i,j,0:51,1])/mass_3R[0])**2+(R_pro[i][j][k][3]-np.sum(Rm[i,j,0:51,2])/mass_3R[0])**2)*Mass[round(R_pro[0][j][k][0])]/mass_3R[0]

        Rg_ij.append(math.sqrt(Rg_n))

        Rg_n=0
        for k in range(51,74):    
            Rg_n=Rg_n+((R_pro[i][j][k][1]-np.sum(Rm[i,j,51:74,0])/mass_3R[1])**2+(R_pro[i][j][k][2]-np.sum(Rm[i,j,51:74,1])/mass_3R[1])**2+(R_pro[i][j][k][3]-np.sum(Rm[i,j,51:74,2])/mass_3R[1])**2)*Mass[round(R_pro[0][j][k][0])]/mass_3R[1]

        Rg_ij.append(math.sqrt(Rg_n))

        Rg_n=0
        for k in range(74,148):
            Rg_n=Rg_n+((R_pro[i][j][k][1]-np.sum(Rm[i,j,74:148,0])/mass_3R[2])**2+(R_pro[i][j][k][2]-np.sum(Rm[i,j,74:148,1])/mass_3R[2])**2+(R_pro[i][j][k][3]-np.sum(Rm[i,j,74:148,2])/mass_3R[2])**2)*Mass[round(R_pro[0][j][k][0])]/mass_3R[2]
...
np.savetxt('Rg_3R.txt', np.c_[Rg_3R],fmt='%f',delimiter='\t')