...
for Per in [0,5,10,15,20,25,30]:
    ...
    f_MSD=open('../Crowd_%d/MSD_100.txt' %(Per),'r')
    line_MSD=f_MSD.readlines()

    MSD.append(0.01*float(line_MSD[0]))
    MSD_std.append(0.01*float(line_MSD[1]))
...
if typeCro=='1':
    ...
    plt.xticks([0.50,0.63,0.76,0.89,1.02],fontproperties="Arial",size=36)
elif typeCro=='2':
    plt.xlim(0,8)
    plt.ylim(0.35,1.0)
    plt.yticks([0.4,0.6,0.8,1.0],fontproperties="Arial",size=36)
    plt.xticks([0.0,2.0,4.0,6.0,8.0],fontproperties="Arial",size=36)


plt.ylabel(chr(961)+'$\mathregular{_{high}}$'+" (g/cm"+'$\mathregular{^3}$)',size=40,labelpad=10,fontproperties="Arial")
plt.xlabel('MSD'+' ($\mathregular{nm^{2}}$)',size=40,labelpad=10,fontproperties="Arial")  
...
plt.savefig('MSD_vs_rou_high.png',dpi=1000,bbox_inches='tight')